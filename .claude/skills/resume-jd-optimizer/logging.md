...
5. Classify each term from what that one Grep returned:
   - **Evidenced in a selected track's Sections 3 to 15 but not itemised in its Section 1**:
     an itemisation gap.
...
- **CDS views** - nice-to-have. Status: evidenced in the Section 5 project text but not
...
Section 12" is useful; "PE licence incomplete" on its own is not.

---

## Master resume sync

The `input/master-resume-*.md` track files are the whole record. There is no combined
master, so a change lands in every track file it belongs in, and the fact sections are meant
...
Writing a fact to only the primary track is the failure mode to avoid. The frozen-facts gate
reads the union of all track files, so the half-applied edit passes on this run and leaves
the files disagreeing for every run after it. Sections 3 and 8 to 11 are identical across
the tracks today; keep them that way. If two sessions both need a sync write, apply one edit
at a time and re-read the file before the second.
