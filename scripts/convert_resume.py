...
# Technical skills are one single-column paragraph per row: the bold category,
# a colon, then its skills, with wrapped lines hanging under the first. This
# is the most parser-safe shape there is. The earlier two-column layout drew
# the category and its skills side by side, which some older ATS parsers read
# column by column, detaching the skills from their category.
#
# The bold is built here with explicit markup rather than through inline(),
# because the row regex strips the markdown asterisks before the label arrives.
...
def skill_block(rows: list[tuple[str, str]]) -> list:
    """The technical skills block: one "Label: values" paragraph per row."""
    block: list = []
    for index, (label, value) in enumerate(rows):
        if index:
            block.append(Spacer(1, SKILL_ROW_GAP))
        markup = f'<b>{escape_xml(label)}:</b> {inline(value)}'
        block.append(Paragraph(markup, SKILL_ROW_STYLE))
    return block
